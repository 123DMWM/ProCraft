...
namespace fCraft.ConfigGUI {
    public sealed partial class MainForm : Form {
        ...
        public MainForm() {
            ...
            Text = "ProCraft Configuration (" + Updater.CurrentRelease.VersionString + ")";
        }
...
Your rank is {RANK}&S. Type &H/Help&S for help." );
       ...
       #region Ranks
        ...
        void SelectRank( Rank rank ) {
            ...
            xUpperCharLimit.Checked = (rank.MaxCaps > 0);
            nUpperCharLimit.Value = rank.MaxCaps;
            ...
            xAllowSecurityCircumvention.Enabled = rank.Can(Permission.ManageWorlds) || rank.Can(Permission.ManageSigns) || rank.Can(Permission.ManageZones) || rank.Can(Permission.ManageSpecialZones);
        ...
        void DisableRankOptions() {
            ...
            nDrawLimit.Value = 0;
            xUpperCharLimit.Checked = false;
            nUpperCharLimit.Value = 0;
        ...
        private void nUpperCharLimit_ValueChanged(object sender, EventArgs e)
        {
            if (selectedRank == null || !xUpperCharLimit.Checked) return;
            selectedRank.MaxCaps = Convert.ToInt32(nUpperCharLimit.Value);
        }
        ...
        private void xUpperCharLimit_CheckedChanged(object sender, EventArgs e)
        {
            if (selectedRank == null) return;
            if (xUpperCharLimit.Checked)
            {
                nUpperCharLimit.Value = selectedRank.MaxCaps;
            }
            else
            {
                nUpperCharLimit.Value = 0;
                selectedRank.MaxCaps = 0;
            }
        ...
        void CheckPermissionConsistency( Permission permission, bool check ) {
            switch( permission ) {
                case Permission.Chat:
                    xUpperCharLimit.Enabled = vPermissions.Items[(int)Permission.Chat].Checked;
                    if( !check ) {
                ...
                case Permission.ManageSigns:
                case Permission.ManageZones:
                case Permission.ManageSpecialZones:
                    xAllowSecurityCircumvention.Enabled = vPermissions.Items[(int)Permission.ManageWorlds].Checked ||
                                                          vPermissions.Items[(int)Permission.ManageSigns].Checked ||
                                                          vPermissions.Items[(int)Permission.ManageZones].Checked ||
                                                          vPermissions.Items[(int)Permission.ManageSpecialZones].Checked;
                    break;
        ...
        void UpdateChatPreview() {
            ...
            lines.Add( String.Format( "{0}{1}Facepalmed&F: This is a normal chat message",
                                      xRankColorsInChat.Checked ? RankManager.HighestRank.Color : "",
                                      xRankPrefixesInChat.Checked ? RankManager.HighestRank.Prefix : "" ) );
            lines.Add( "&Pfrom 123DMWM: This is a private message / whisper" );
            lines.Add( "&M*Facepalmed is using /Me to write this" );
            if( xShowJoinedWorldMessages.Checked ) {
                ...
                lines.Add( String.Format( "&SPlayer {0}{1}Facepalmed&S joined {2}{3}SomeOtherMap",
                                          xRankColorsInChat.Checked ? RankManager.HighestRank.Color : "",
                                          xRankPrefixesInChat.Checked ? RankManager.HighestRank.Prefix : "",
            ...
            if( xAnnounceKickAndBanReasons.Checked ) {
                lines.Add( String.Format( "&W{0}{1}123DMWM&W was kicked by {0}{1}Facepalmed&W: Reason goes here",
                                          xRankColorsInChat.Checked ? RankManager.HighestRank.Color : "",
                                          xRankPrefixesInChat.Checked ? RankManager.HighestRank.Prefix : "" ) );
            } else {
                lines.Add( String.Format( "&W{0}{1}123DMWM&W was kicked by {0}{1}Facepalmed",
                                          xRankColorsInChat.Checked ? RankManager.HighestRank.Color : "",
                                          xRankPrefixesInChat.Checked ? RankManager.HighestRank.Prefix : "" ) );
            }
