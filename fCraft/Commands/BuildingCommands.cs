...
namespace fCraft {
    ...
    static class BuildingCommands {
        ...
        internal static void Init() {
            ...
            CommandManager.RegisterCommand( CdTotalBan );
        ...
        private static void FuncHandler(Player player, CommandReader cmd)
        {
            FuncDrawOperation operation = new FuncDrawOperationPoints(player, cmd);
            DrawOperationBegin(player, cmd, operation);
        }

        private static void FuncSHandler(Player player, CommandReader cmd)
        {
            FuncDrawOperation operation = new FuncDrawOperationSurface(player, cmd);
            DrawOperationBegin(player, cmd, operation);
        }

        private static void FuncFHandler(Player player, CommandReader cmd)
        {
            try
            {
        ...
        private static void InequalityHandler(Player player, CommandReader cmd)
        {
            try
            {
        ...
        private static void EqualityHandler(Player player, CommandReader cmd)
        {
            try
            {
        ...
        private static void StartParametrizedDraw(Player player, CommandReader cmd)
        {
            try
            {
        ...
        private static void CenterCallback(Player player, Vector3I[] marks, object tag)
        {
            if (player.LastUsedBlockType != Block.None)
            {
                ...
                {
                    Vector3I cPos = new Vector3I(((bounds.XMin + bounds.XMax) / 2) + 1,
                ...
                Vector3I cPoso = new Vector3I((bounds.XMin + bounds.XMax) / 2,
                    (bounds.YMin + bounds.YMax) / 2,
                    (bounds.ZMin + bounds.ZMax) / 2);
        ...
        #endregion
        #region TOTALFUCKERBANER

        private static readonly CommandDescriptor CdTotalBan = new CommandDescriptor
        {
            Name = "TotalBan",
            Aliases = new[] { "tb" },
            Category = CommandCategory.New,
            IsConsoleSafe = false,
            IsHidden = false,
            Permissions = new[] { Permission.TotalBan },
            Usage = "/TotalBan playerName reason",
            Help = "Bans IP(including every player on it), demotes the target player, and undoes the target players block changes",
            Handler = BanXHandler
        };

        private static void BanXHandler(Player player, CommandReader cmd)
        {
            string ban = cmd.Next();
            ...
            if (ban == null)
            {
                player.Message("&WError: Enter a player name to TotalBan");
                return;
            }
            if (reason.Length <= 2)
            {
                player.Message("&WError: Please enter a reason for this TotalBan");
                return;
            }
            ...
            PlayerInfo target = PlayerDB.FindPlayerInfoOrPrintMatches(player, ban, SearchOptions.Default);
            if (target == null)
                return;
            if (!Player.IsValidPlayerName(ban))
            {
                CdTotalBan.PrintUsage(player);
                return;
            }
            else
            {
                try
                {
                    ...
                    target.BanAll(player, reason, false, true);
                }
                ...
                if (player.Can(Permission.Demote, target.Rank))
                {
                    if (target.Rank != RankManager.LowestRank)
                    {
                        ...
                        player.Message("{0} &swas demoted to {1}",target.ClassyName , RankManager.LowestRank.ClassyName);
                    }
                    Server.Players.Message("{0}&S was TotalBanned by {1}&S Reason:&W {2}", target.ClassyName, player.ClassyName, reason);
                    string message = String.Format("\u212C&WPlayer {0}&W was TotalBanned by {1}&W Reason: {2}",
                                            target.ClassyName,
                                            player.ClassyName,
                                            reason);
                ...
                else
                {
                    ...
                    Server.Players.Message("{0}&S was TotalBan'd by {1}: &W{2}", target.ClassyName, player.ClassyName, reason);
                    Server.Players.Message("{0}&S was TotalBan'd by {1}&S Reason:&W {2}", target.ClassyName, player.ClassyName, reason);
                    string message = String.Format("\u212C&WPlayer {0}&W was TotalBanned by {1}&W Reason: {2}",
                                            target.ClassyName,
                                            player.ClassyName,
                                            reason);
        ...
        {
            BlockDBUndoArgs args = new BlockDBUndoArgs() { Player = player, World = player.World, CountLimit = 2147483647, Not = false, Targets = target };
