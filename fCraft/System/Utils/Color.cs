...
#pragma warning restore 1591
        ...
        public static string[] LogColors([NotNull] string input, char initialcolor)
        {
            List<string> output = new List<string>();
            string nextoutput = "";
            char nextcolor = initialcolor;
            ...
            else
            {
                for (int i = 0; i < input.Length; i++)
                {
                    if (input[i] == '&')
                    {
                        if (input.Length > i)
                        {
                            foreach (char colorid in Color.ColorNames.Keys)
                            {
                                if (input[i + 1] == colorid)
                                {
