                ...
                for (; imageY <= maxY; imageY++)
                {
                    ...
                    for (; layer < Palette.Layers; layer++)
                    {
                        ...
                        if (DrawOneBlock())
                        {
                            ...
                            //Block oldBlock = Player.WorldMap.GetBlock(placetobuild.XMin, placetobuild.YMin, placetobuild.ZMin);
                            //if (oldBlock != Block.None)
                            //{
                            //    Player.Send(Packet.MakeMakeSelection(1, "DrawImage", placetobuild, rtn, 255));
                            //    Player.Send(Packet.MakeRemoveSelection(1));
                            //}                            
