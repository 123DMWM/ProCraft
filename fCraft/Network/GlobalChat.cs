    ...
    public static class GlobalChat {
                ...
                do {
                    ...
                    try {
                        ...
                        Logger.Log( LogType.SystemActivity,
                                    "Connecting to ProCraft Global Chat as {2}",
                                    hostName, port, ActualBotNick );
                        if (ActualBotNick == "Custom Minecraft Server (ProCraft)") {
            ...
            void HandleMessage( [NotNull] string message ) {
                ...
                switch (msg.Type) {
                    ...
                    case IRCMessageType.ChannelMessage:
                        ...
                        processedMessage = IRC.NonPrintableChars.Replace( Color.IrcToMinecraftColors(processedMessage), "" );
                        if (processedMessage.Length > 0) {
                            if (msg.Type == IRCMessageType.ChannelAction) {
                                SendList.Message( "&i[Global] * &m{1} {2}", ActualBotNick, msg.Nick, processedMessage );
                                Logger.Log( LogType.GlobalChat, "[Global] * {1} {2}", ActualBotNick, msg.Nick, processedMessage );
                            } else {
                                SendList.Message( "&i[Global] {1}&7: {2}", ActualBotNick, msg.Nick, processedMessage );
                                Logger.Log( LogType.GlobalChat, "[Global] {1}: {2}", ActualBotNick, msg.Nick, processedMessage );
                            }
                        } else if (msg.Message.StartsWith( "#" )) {
                            SendList.Message( "&i[Global] {1}&7: {2}", ActualBotNick, msg.Nick, processedMessage.Substring( 1 ) );
                    ...
                    case IRCMessageType.Join:
                        ...
                        if (msg.Nick.StartsWith( "(" )) {
                            SendList.Message( "&i[Global] Server {0} joined the ProCraft Global Chat", msg.Nick );
                            Logger.Log( LogType.GlobalChat, "[Global] Server {0} joined the ProCraft Global Chat", msg.Nick );
                        } else {
                            SendList.Message( "&i[Global] {0} joined the ProCraft Global Chat", msg.Nick );
                            Logger.Log( LogType.GlobalChat, "[Global] {0} joined the ProCraft Global Chat", msg.Nick );
                        }
                        return;
                    ...
                    case IRCMessageType.Quit:
                        if (!ResponsibleForInputParsing)
                            return;
                        SendList.Message( "&i[Global] Server {0} left the ProCraft Global Chat", msg.Nick );
                        Logger.Log( LogType.GlobalChat, "[Global] Server {0} left the ProCraft Global Chat", msg.Nick );
                        return;
                    ...
                    case IRCMessageType.Error:
                        ...
                        switch (msg.ReplyCode) {
                            ...
                            case IRCReplyCode.ErrorBadChannelKey:
                                Logger.Log( LogType.SystemActivity,
                                            "Error: Channel password required for {0}. ProCraft does not currently support passworded channels.",
                                            msg.Channel );
                                die = true;
                                GCReady = false;
                                break;
        ...
        public static void Init() {
            ...
            if (serverName.Length > 28) {
                serverName = serverName.Substring( 0, 28 );
            }
