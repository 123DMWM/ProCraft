    ...
    public static class PlayerEnumerable {
        ...
        public static int Message( [NotNull] this IEnumerable<Player> source,
            ...
            foreach (Player player in sourceArray) {
                foreach (Packet packet in LineWrapper.Wrap(message, player.Supports(CpeExtension.EmoteFix), player.Supports(CpeExtension.FullCP437))) {
                    player.Send(packet);
                    i++;
                }
            }
        ...
        public static int MessagePrefixed( [NotNull] this IEnumerable<Player> source, [NotNull] string prefix,
            ...
            Player[] sourceArray = source.ToArray();
            foreach (Player player in sourceArray) {
                foreach (Packet packet in LineWrapper.WrapPrefixed(prefix, message, player.Supports(CpeExtension.EmoteFix), player.Supports(CpeExtension.FullCP437))) {
                    player.Send(packet);
                    i++;
                }
            }
