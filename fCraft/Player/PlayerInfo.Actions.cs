...
namespace fCraft {
    public sealed partial class PlayerInfo {
                ...
                if( result ) {
                    ...
                    if( target != null ) {
                        ...
                        if( !unban ) {
                            string kickReason;
                            if( reason != null ) {
                                kickReason = String.Format( "&fBanned by {0}&f: {1}", player.Name, reason );
                            } else {
                                kickReason = String.Format( "&fBanned by {0}", player.Name );
                            }
            ...
            lock( actionLock ) {
                ...
                if( needIPUnban ) {
                    if( IPBanList.Remove( address, raiseEvents ) ) {
                        ...
                        if( announce ) {
                            var can = Server.Players.Can( Permission.ViewPlayerIPs );
                            can.Message( "{0}&W was IP-unbanned ({1}) by {2}",
                                         ClassyName, address, player.ClassyName );
                            var cant = Server.Players.Cant( Permission.ViewPlayerIPs );
                            cant.Message( "{0}&W was IP-unbanned by {1}",
                                          ClassyName, player.ClassyName );
                            if( ConfigKey.AnnounceKickAndBanReasons.Enabled() && reason != null ) {
            ...
            lock( actionLock ) {
                ...
                foreach( PlayerInfo targetAlt in allPlayersOnIP ) {
                    ...
                    if( targetAlt.ProcessBan( player, player.Name, reason ) ) {
                        ...
                        if( announce ) {
                            if( targetAlt == this ) {
                                Server.Message( "{0}&W was banned by {1}&W (BanAll)",
                                                targetAlt.ClassyName, player.ClassyName );
                            } else {
                                Server.Message( "{0}&W was banned by {1}&W by association with {2}",
            ...
            lock( actionLock ) {
                ...
                foreach( PlayerInfo targetAlt in allPlayersOnIP ) {
                    ...
                    if( targetAlt.ProcessUnban( player.Name, reason ) ) {
                        ...
                        if( announce ) {
                            if( targetAlt == this ) {
                                Server.Message( "{0}&W was unbanned by {1}&W (UnbanAll)",
                                                targetAlt.ClassyName, player.ClassyName );
                            } else {
                                Server.Message( "{0}&W was unbanned by {1}&W by association with {2}",
                                                targetAlt.ClassyName, player.ClassyName, ClassyName);
                            }
                        }
                        somethingGotUnbanned = true;
        ...
        public void TempBan([NotNull] Player player, TimeSpan duration, [CanBeNull] string reason, bool announce, bool raiseEvents)
        {
            if (player == null) throw new ArgumentNullException("player");
        ...
        /// <param name="raiseEvents"> Whether to raise PlayerInfo.MuteChanging and MuteChanged events. </param>
        public void UnTempBan([NotNull] Player player, TimeSpan duration, [CanBeNull] string reason, bool announce, bool raiseEvents)
        {
            if (player == null) throw new ArgumentNullException("player");
        ...
        public void ChangeRank( [NotNull] Player player, [NotNull] Rank newRank, [CanBeNull] string reason,
            ...
            {
                if (this.RankChangedBy == player.Name)
                {
                    if (promoting)
                    {
                        if (this.RankChangeType == RankChangeType.Promoted)
                        {
                            player.Info.PromoCount++;
                        }
                        else if (this.RankChangeType == RankChangeType.Demoted)
                        {
                            player.Info.PromoCount++;
                            player.Info.DemoCount--;
                        }
                    }
                    if (!promoting)
                    {
        ...
        public void Freeze( [NotNull] Player player, bool announce, bool raiseEvents ) {
            ...
            lock( actionLock ) {
                ...
                if( announce ) {
                    ...
                    Server.Message( target, "{1}&s successfully &bfroze&s: {1}",
                                            ClassyName, player.ClassyName );
                }
        ...
        public void Unfreeze( [NotNull] Player player, bool announce, bool raiseEvents ) {
            ...
            lock( actionLock ) {
                ...
                if( announce ) {
                    Player target = PlayerObject;
                    if( target != null ) {
                        target.Message( "&WYou were thawed out by {0}", player.ClassyName );
                    }
                    Server.Message( target, "{0}&S was thawed out by {1}",
                                            ClassyName, player.ClassyName );
                }
